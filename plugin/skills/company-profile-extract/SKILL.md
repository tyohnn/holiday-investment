...
- `period_key`: 기수(제N기) 기반 표는 본문의 `'YYYY년(제N기)'` 앵커 문장에서 연도를
  산술로 역산한다(`infer_period_labels`) — 기수→연도 표를 하드코딩하지 않는다. 주주·
  자사주처럼 표 자체에 연도 라벨이 없는 시점형 개념은 `filings.report_nm`(`사업보고서
  (YYYY.MM)`)에서 그 rcept_no 의 회계연도를 가져와 채운다.
...
- **부문별 매출 표(`부 문/매출유형/품 목` 헤더) 구조가 다른 회사**는 `parse_segment_revenue`
  ...
  concept별 카운트가 0으로 보고된다 — 실행 후 커버리지 보고를 반드시 확인한다).
