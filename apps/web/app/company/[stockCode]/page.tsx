...
export async function generateMetadata(props: PageProps<'/company/[stockCode]'>): Promise<Metadata> {
  const { stockCode } = await props.params;
  return { title: `종목 1페이지 · ${stockCode}` };
}

export default async function CompanyPage(props: PageProps<'/company/[stockCode]'>) {
  const { stockCode } = await props.params;
