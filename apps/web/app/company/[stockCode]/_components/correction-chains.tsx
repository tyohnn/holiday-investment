...
function daysBadgeClass(days: number | null): string {
  if (days === null) return 'bg-fd-secondary text-fd-secondary-foreground';
      ...
      <ul className="mt-3 space-y-2">
        {corrections.map((c) => (
          <li
            ...
            className="flex flex-col gap-2 rounded-xl border border-fd-border bg-fd-card p-4 sm:flex-row sm:items-center sm:justify-between"
          >
            <div className="min-w-0">
              <div className="truncate text-sm font-medium">{c.base_report_nm}</div>
              <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-fd-muted-foreground">
