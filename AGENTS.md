...
- **메인(감독)** — 채(sieve) 설계, peer set 확정, 서브에이전트 프롬프트 작성, **산출물 검증**,
  기법 간 입력값 연결(#3 산출 → #7 입력 → #6 계산), 최종 종합. 직접 분석 노동을 하지 않는다.
...
```
원격 실측(2026-08 기준): companies 3,978 · filings 2,086,950 · financial_facts 13,920,062 ·
report_items 1,248,503 · ownership_txns 18,961 · events 7,387. 공시는 2026-07-29까지 최신.
...
- `financial_facts.sj_div`에서 손익은 **회사마다 `IS`(손익계산서)이거나 `CIS`(포괄손익계산서)다.**
  크래프톤은 CIS 뿐이고 LG에너지솔루션·LG화학은 IS와 CIS를 둘 다 낸다. **한쪽만 조회하면
  대형사가 통째로 빈다** — 반드시 `sj_div=in.(IS,CIS)`로 긁고, 둘 다 있으면 본표인 IS를 쓴다.
- `reprt_code`: 11011=사업보고서(연간), 11012=반기, 11013=1분기, 11014=3분기.
  분기 계열의 `amount`는 누적이 아니라 **단일 분기**로 적재돼 있다(크래프톤에서 교차검증).
- **매출·영업이익 계정명은 회사마다 다르다.** "매출액" / "영업수익" / "수익(매출액)",
  영업이익은 "영업이익" / "영업이익(손실)" / "영업손익" / "Ⅲ.영업손익". `account_nm` 실제
  값을 먼저 뽑아 확인한다. **부분문자열 매칭은 오폭한다** — LG화학은 `영업수익`(45.9조)과
  `기타영업수익`(1.6조)을 둘 다 내므로 정확일치를 먼저 시도해야 한다.
- **연결(CFS)이 아예 없는 회사가 있다.** 종속회사가 없으면 별도(OFS)만 존재한다
  (에코프로머티 등). CFS가 0행이면 OFS로 폴백하고 기준을 표기한다.
...
| 1 | #1 능력범위 + #3 산업 — 기업이 아니라 **섹터를 판정** | DB + 웹 |
| 2 | 정량 스크린 — 매출성장·영업이익률 4~5년 추이 (기계) | DB |
| 3 | #2 해자·가격결정권 — peer 이익률 비교 | DB + 웹 |
| 4 | #4 피셔 — 경영진·재무제표 밖 → **후보 20~30 확정** | 웹 + 유튜브 |
| 5 | #7 → #6 → #10 — 후보 전수 적정가·매수가 역산 | DB + 웹 |
| 6 | #12·#13 1차자료·뉴스 반증 — 상위 후보만 | DB + 웹 + 유튜브 |

채가 좁아질수록 소스가 DB → 웹 → 유튜브로 옮겨간다. 비용이 그 순서로 오르기 때문이다.

### 산출물 규칙
- 모든 숫자에 출처: DB는 `(DB: financial_facts, 2025, CIS, 영업이익)`, 웹은 URL,
